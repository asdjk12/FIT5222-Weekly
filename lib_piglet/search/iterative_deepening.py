...
class iterative_deepening(base_search):
    ...
    def get_path(self, start_state, goal_state, threshold_type=ID_threshold.depth):
        ...
        while self.runtime_ < self.time_limit_:
            ...
            if threshold_type == ID_threshold.depth:
                next_threshold = solution[1]
            else: 
            # cost limition rely on f-value
                next_threshold = solution[2]
            ...
            if solution[0] is None:
                if next_threshold == sys.maxsize: #or threshold_type == ID_threshold.cost and next_cost >= sys.maxsize :
                    self.solution_ = None
                    self.status_ = "Failed"
                    return None
                if  threshold_type == ID_threshold.depth:
                    depth_threshold = solution[1]
                else: 
                # cost limition rely on f-value
                    cost_threshold = solution[2]
